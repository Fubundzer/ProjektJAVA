...
public class Katalog<TYP> {
	...
	{
		int i;
		cout << "W ktore miejsce chcesz wstawic nowy album?" << endl;
		cin >> i;
		while (cin.fail()||i>dane.size()+1||i<1)
		{
			getZnakDlError();
			Sleep(1000);
			cin.clear();
	...
	template <class TYP>
	Katalog<TYP> Katalog<TYP>::wstawianietab()
	{
		int i, j;
		TYP *tab;
	...
	template <class T> T dodawaniavec()
	{
		...
		cin.sync();
		getline(cin, tytul);
		cout << "Podaj gatunek plyty: ";
		cin.sync();
		getline(cin, gatunek);
		cout << "Podaj ilosc piosenek: ";
		cin >> ilosc_p;
		while (cin.fail())
		{
			getZnakError();
		...
		T a(autor, tytul, gatunek, ilosc_p, data);
		return a;
	}
